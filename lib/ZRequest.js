...
const postsLoadersPath = path.join(process.cwd(), 'lib/postsLoader');
const postsLoaders = fs.readdirSync(postsLoadersPath)
	.reduce(function (L, file) {
		const re = /^([\w\-]+)[\.]js$/i;
		if (re.test(file) && file !== 'index.js') {
			const key = file.replace(re, '$1');
			const value = uf('./postsLoader/%s', file);
			L[key] = require(value);
		}
		return L;
	}, {});
const queriesLoader = require('./queriesLoader');
const request2dataPath = path.join(process.cwd(), 'lib/request2data');
const request2data = fs.readdirSync(request2dataPath)
	.reduce(function (D, file) {
		const re = /^([\w]+)[\.]js$/i;
		if (re.test(file)) {
			const key = file.replace(re, '$1');
			const value = uf('./request2data/%s', file);
			D[key] = require(value);
		}
		return D;
	}, {});
