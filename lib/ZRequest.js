...
const postsLoaders = fs.readdirSync(uf('%s/postsLoader', __dirname))
	.reduce(function (L, file) {
		const re = /^([\w\-]+)[\.]js$/i;
		if (re.test(file) && file !== 'index.js') {
			const key = file.replace(re, '$1');
			const path = uf('./postsLoader/%s', file);
			L[key] = require(path);
		}
		return L;
	}, {});
const queriesLoader = require('./queriesLoader');
const request2data = fs.readdirSync(uf('%s/request2data', __dirname))
...
class ZRequest {
	...
	load(options) {
// TODO: split load for cookies, queries and posts + uploads
		const Z = this;
		...
		return Promise.resolve()
			...
			.then(function () {
				...
				else if (type in postsLoaders) {
					return postsLoaders[type](Z, O);
				}
				else {
					return postsLoader(Z, O);
				}
			})
			.then(function (pu) {
				if ('posts' in pu) {
