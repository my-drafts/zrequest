...
const bindCookie = require('./bindCookie');
const bindPost = require('./bindPost');
const bindQuery = require('./bindQuery');
const bindSession = require('./bindSession');
const bindUpload = require('./bindUpload');
...
const ZRequestQuery = require('./ZRequestGet');
const ZRequestPost = require('./ZRequestPost');
//const ZRequestSession = require('./ZRequestSession');
...
class ZRequest {
	...
	constructor(request) {
		if (zt.as(request).has('IncomingMessage')) {
			...
			this.SESSION = {};
			this.UPLOAD = {};
			bindCookie(this, 'COOKIE');
			bindPost(this, 'POST');
			bindQuery(this, 'QUERY');
			bindSession(this, 'SESSION');
			bindUpload(this, 'UPLOAD');
	...
	init(options) {
		...
		return Promise.resolve()
			...
			.then(function () {
				if (false) {
					return ZRequestSession(R, O.session);
				}
				else {
					return Promise.resolve();
				}
