...
const bindUploadClean = function (zrequest, property, call, freeze) {
  const R = zt.al(zrequest, {
    o: zrequest,
    else: {}
  });
  R[call] = function (options) {
    ...
    const promise = Object.keys(R[property]).map(function (field) {
      return R[property][field];
    }).reduce(function (all, files) {
      return all.concat.apply(all, files);
...
class ZRequest {
  ...
  constructor(request) {
    if (zt.as(request).has('IncomingMessage')) {
      ...
      bindUploadClean(this, 'UPLOAD', 'uploadClean', false);
    }
    else {
      throw 'wrong request';
    }
